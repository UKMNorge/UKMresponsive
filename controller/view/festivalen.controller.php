...
// HENT ALLE POSTS
    ...
    $posts = query_posts('posts_per_page=12&cat=1&paged='.$paged);
    while(have_posts()) {
        the_post();
...
$sql = new SQL("SELECT blog_url, post_meta FROM ukmno_wp_related WHERE blog_id = '#blog_id' AND post_type = 'image' ORDER BY rel_id DESC LIMIT 40", array('blog_id' => $blog_id));
$res = $sql->run();
